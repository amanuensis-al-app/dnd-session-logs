...
import { parseCsv } from './importAlLog';
...
4. PurchaseLogEntry → shopping recorded as free-text notes bullets. Each bullet is either a BUY or a SELL — they must NOT end up in the same log:
   ...
   - EXCEPTION, character creation: the character's FIRST entry, when its notes mention starting equipment or starting gold (e.g. "Wizard Starting Equipment: 55GP" / "Custom Background Starting Equipment: 50 GP"), is NOT a purchase — make ONE "creation" log titled "Character Creation": gpGained = the column value, and any listed starting gear as equipment itemsGained (no costs).
...
- "name" is ONLY the plain D&D item name. Never leave quantities, prices or verbs in it:
  - "* 10 Parchment = 1GP" → name "Parchment", quantity 10, cost 0.1 (a bullet's price is the LINE TOTAL: divide by the quantity).
  - "* Acid (Vial x1)" → name "Acid", quantity 1. "(x1)", "x20", "2x" anywhere mark the quantity.
...
- Keep each entry's notes text VERBATIM in "notes" (including bullets you parsed items from; use \\n for line breaks) — except sell lines, which move to the sell log's notes. Sessions also get the "DM DCI: ..." line. When session_num, xp_gained or renown_gained are non-empty/non-zero, append lines "Session #N", "XP gained: X", "Renown gained: X".
